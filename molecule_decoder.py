...
try:
    ...
    from rdkit.Chem import QED, rdmolops
...
def generate_bond_pairs(max_atoms: int) -> List[Tuple[int, int]]:
    ...
    for j in range(max_atoms - 1):
        for i in range(j + 1, max_atoms):
            pairs.append((j, i))
    return pairs
...
class MoleculeDecoder:
    ...
    def decode_bitstring(self, bitstring: str, count: int = 0) -> Dict:
        ...
        if mol is not None:
            ...
            if smiles is not None:
                ...
                record['qed'] = self.compute_qed(mol)
                record['valid'] = True
                record['mol'] = mol
                return record
        ...
        if n_decoded_atoms >= 2:
            ...
            if salvage is not None:
                s_mol, s_smiles, s_qed, coverage = salvage
    ...
    def _try_salvage(
        ...
        return best_frag, smiles, self.compute_qed(best_frag), coverage
    ...
    def compute_qed(mol: Chem.Mol) -> float:
        if not RDKIT_AVAILABLE:
            return 0.0
        try:
            return float(QED.qed(Chem.AddHs(mol)))
        except Exception:
            return 0.0
    ...
    # 修改原因：validity 改為 shot-weighted 定義，uniqueness 保持有效樣本種類比率。
    def compute_fitness(self, counts: Dict[str, int]) -> Tuple[Tuple[float, float], List[Dict]]:
        try:
            decoded = self.decode_counts(counts)
        except Exception:
            return (0.0, 0.0), []

        # [FIX-P1-1] 修正 Validity 分母：改用 shots 總次數（論文 Eq. 4 定義）
        total_shots = sum(counts.values())
        valid = [
            r for r in decoded
            if r.get('valid') and not r.get('partial_valid')
        ]
        # [FIX-P1-1] 有效 shots 數 = 所有有效 bit-string 各自的出現次數之和
        valid_shots = sum(
        ...
        return (float(validity), float(uniqueness)), decoded
    ...
    def summarize(self, decoded_results: List[Dict]) -> str:
        ...
        if valid:
            qeds = [record['qed'] for record in valid]
            lines.append(f'  Mean QED (valid)    : {sum(qeds) / len(qeds):.4f}')
            lines.append(f'  Max  QED (valid)    : {max(qeds):.4f}')
